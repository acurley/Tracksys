class Order < ActiveRecord::Base
  ...
  before_save do
    ...
    self.is_approved = 1 if order_status == 'approved'
  end

  # Returns a string containing a brief, general description of this
  # class/model.
  def self.class_description
    'Order represents an order for digitization, placed by a Customer and made up of one or more Units.'
  end

  def self.entered_by_description
    'ID of person who filled out the public request form on behalf of the Customer.'
  end
  ...
  def active?
    if order_status == 'canceled' || order_status == 'deferred'
      return false
    else
      return true
    end
  ...
  def has_units_being_prepared
    units_beings_prepared = Unit.where(order_id: id).where('unit_status = "unapproved" or unit_status = "condition" or unit_status = "copyright"')
    units_beings_prepared
  end
  ...
  def validate_order_approval
    units_beings_prepared = has_units_being_prepared
    unless units_beings_prepared.empty?
  ...
  def self.due_within(timespan)
    unless timespan.is_a?(ActiveSupport::TimeWithZone)
      logger.error "#{name}#due_within expecting ActiveSupport::TimeWithZone as argument.  Got #{timespan.class} instead"
  ...
  def self.overdue_as_of(date = 0.days.ago)
    unless date.is_a?(ActiveSupport::TimeWithZone)
      logger.error "#{name}#overdue_as_of Expecting ActiveSupport::TimeWithZone as argument. Got #{date.class} instead"
      date = 0.days.ago
    end
  ...
  def ready_to_approve?
    status = units.map(&:unit_status) & %w(condition copyright unapproved)
    status.empty?
  end
  ...
  def approve_order
    message = ActiveSupport::JSON.encode(order_id: id)
  ...
  def create_order_pdf
    message = ActiveSupport::JSON.encode(order_id: id, fee: fee_actual.to_i)
  ...
  def send_fee_estimate_to_customer(computing_id)
    ...
    message = ActiveSupport::JSON.encode(order_id: id, first_name: @first_name)
