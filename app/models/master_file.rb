class MasterFile < ActiveRecord::Base
  ...
  def next
    master_files_sorted = sorted_set
  ...
  def sorted_set
    master_files_sorted = unit.master_files.sort_by(&:filename)
  end

  def link_to_dl_thumbnail
    "http://fedoraproxy.lib.virginia.edu/fedora/get/#{pid}/djatoka:jp2SDef/getRegion?scale=125"
  end

  def link_to_dl_page_turner
    "#{VIRGO_URL}/#{bibl.pid}/view?&page=#{pid}"
  end

  def path_to_archved_version
    "#{archive.directory}/" + "#{'%09d' % unit_id}/" + "#{filename}"
  end

  def link_to_static_thumbnail
    thumbnail_name = filename.gsub(/(tif|jp2)/, 'jpg')
    ...
    "/metadata/#{@range_dir}/#{unit_dir}/Thumbnails_(#{unit_dir})/#{thumbnail_name}"
  end
  ...
  def get_from_stornext(computing_id)
    message = ActiveSupport::JSON.encode(workflow_type: 'patron', unit_id: unit_id, master_file_filename: filename, computing_id: computing_id)
    publish :copy_archived_files_to_production, message
  end

  def update_thumb_and_tech
    image_tech_meta.destroy if image_tech_meta
    sleep(0.1)

    message = ActiveSupport::JSON.encode(master_file_id: id, source: path_to_archved_version)
  ...
  def increment_counter_caches
    ...
    Customer.increment_counter('master_files_count', customer.id)
    Order.increment_counter('master_files_count', order.id)
  end
  ...
  def decrement_counter_caches
    ...
    Customer.decrement_counter('master_files_count', customer.id)
    Order.decrement_counter('master_files_count', order.id)
  end
end
