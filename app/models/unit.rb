class Unit < ActiveRecord::Base
  ...
  validates :patron_source_url, format: { with: URI.regexp(%w(http https)) }, allow_blank: true
  ...
  def next
    units_sorted = order.units.sort_by(&:id)
  ...
  def check_unit_delivery_mode
    message = ActiveSupport::JSON.encode(unit_id: id)
    publish :check_unit_delivery_mode, message
  end

  def get_from_stornext(computing_id)
    message = ActiveSupport::JSON.encode(workflow_type: 'patron', unit_id: id, computing_id: computing_id)
    publish :copy_archived_files_to_production, message
  end

  def import_unit_iview_xml
    unit_dir = '%09d' % id
    message = ActiveSupport::JSON.encode(unit_id: id, path: "#{IN_PROCESS_DIR}/#{unit_dir}/#{unit_dir}.xml")
    publish :import_unit_iview_xml, message
  end

  def qa_filesystem_and_iview_xml
    message = ActiveSupport::JSON.encode(unit_id: id)
    logger.tagged('MS3UF') { logger.debug "model method Unit#qa_filesystem_and_iview_xml called on #{id}" }
  ...
  def queue_unit_deliverables
    @unit_dir = '%09d' % id
    message = ActiveSupport::JSON.encode(unit_id: id, mode: 'patron', source: File.join(PROCESS_DELIVERABLES_DIR, 'patron', @unit_dir))
    publish :queue_unit_deliverables, message
  end

  def send_unit_to_archive
    message = ActiveSupport::JSON.encode(unit_id: id, internal_dir: 'yes', source_dir: "#{IN_PROCESS_DIR}")
    publish :send_unit_to_archive, message
  end

  def start_ingest_from_archive
    message = ActiveSupport::JSON.encode(unit_id: id, order_id: order_id)
  ...
  def copy_metadata_to_metadata_directory
    ...
    message = ActiveSupport::JSON.encode(unit_id: id, unit_path: unit_path)
  ...
  def qr
    code = RQRCode::QRCode.new("#{TRACKSYS_URL}/admin/unit/checkin/#{id}", size: 7)
    code
  end
  ...
  def fill_in_missing_transcriptions
    text = " \n"
    empty_items = master_files.where(transcription_text: nil)
  ...
  def ready_for_repo?
    if include_in_dl == true && !availability_policy_id.nil? && date_queued_for_ingest.nil? && !date_archived.nil?
      return true
    else
      return false
    end
