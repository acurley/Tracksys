class Invoice < ActiveRecord::Base
  ...
  def self.outstanding_as_of(date = 0.days.ago)
    unless date.is_a?(ActiveSupport::TimeWithZone)
      logger.error "#{name}#outstanding_as_of Expecting ActiveSupport::TimeWithZone as argument. Got #{date.class} instead"
      date = 0.days.ago
    end
  ...
  def self.second_notice_as_of(date = 0.days.ago)
    unless date.is_a?(ActiveSupport::TimeWithZone)
      logger.error "#{name}#second_notice_as_of Expecting ActiveSupport::TimeWithZone as argument. Got #{date.class} instead"
      date = 0.days.ago
    end
  ...
  scope :permanent_nonpayment, -> { where('permanent_nonpayment != 0') }
