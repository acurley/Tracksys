ActiveAdmin.register AcademicStatus do
  ...
  index do
    ...
    column :customers do |academic_status|
      link_to "#{academic_status.customers_count}", admin_customers_path(q: { academic_status_id_eq: academic_status.id })
    end
    column :requests do |academic_status|
      link_to "#{academic_status.requests.size}", admin_orders_path(q: { academic_status_id_eq: academic_status.id }, scope: 'awaiting_approval')
    end
    column :orders do |academic_status|
      link_to "#{academic_status.orders.size}", admin_orders_path(q: { academic_status_id_eq: academic_status.id }, scope: 'approved')
    end
    column :units do |academic_status|
      link_to "#{academic_status.units.size}", admin_units_path(q: { academic_status_id_eq: academic_status.id })
    end
    column :master_files do |academic_status|
      link_to "#{academic_status.master_files.size}", admin_master_files_path(q: { academic_status_id_eq: academic_status.id })
    end
