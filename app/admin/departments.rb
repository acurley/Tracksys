ActiveAdmin.register Department do
  ...
  index do
    ...
    column :customers do |department|
      link_to "#{department.customers.size}", admin_customers_path(q: { department_id_eq: department.id })
    end
    column :requests do |department|
      link_to "#{department.requests.size}", admin_orders_path(q: { department_id_eq: department.id, scope: 'awaiting_approval' })
    end
    column :orders do |department|
      link_to "#{department.orders.size}", admin_orders_path(q: { department_id_eq: department.id, scope: 'approved' })
    end
    column :units do |department|
      link_to "#{department.units.size}", admin_units_path(q: { department_id_eq: department.id })
    end
    column :master_files do |department|
      link_to "#{department.master_files.size}", admin_master_files_path(q: { department_id_eq: department.id })
    end
