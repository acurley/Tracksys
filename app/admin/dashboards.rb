  ...
  content do
    div class: 'three-column' do
      panel "Active Errors (#{AutomationMessage.has_active_error.count})", namespace: :admin, priority: 1, toggle: 'show' do
        table do
          tr do
            td { 'Archiving Workflow' }
            td { link_to "#{AutomationMessage.archive_workflow.has_active_error.count}", admin_automation_messages_path(q: { active_error_eq: true }, scope: 'archive') }
          end
          tr do
            td { 'QA Errors' }
            td { link_to "#{AutomationMessage.qa_workflow.has_active_error.count}", admin_automation_messages_path(q: { active_error_eq: true }, scope: 'qa') }
          end
          tr do
            td { 'Delivery Errors' }
            td { link_to "#{AutomationMessage.delivery_workflow.has_active_error.count}", admin_automation_messages_path(q: { active_error_eq: true }, scope: 'delivery') }
          end
          tr do
            td { 'Patron Approval Errors' }
            td { link_to "#{AutomationMessage.patron_workflow.has_active_error.count}", admin_automation_messages_path(q: { active_error_eq: true }, scope: 'patron') }
          end
          tr do
            td { 'Repository Workflow Errors' }
            td { link_to "#{AutomationMessage.repository_workflow.has_active_error.count}", admin_automation_messages_path(q: { active_error_eq: true }, scope: 'repository') }
          end
        end
      end
    end
    ...
    div class: 'three-column' do
      panel 'Finalization', width: '33%', namespace: :admin, priority: 2, toggle: 'show' do
        table do
          tr do
            td { 'Unfinished Units of Partially Finalized Orders' }
            td { link_to "#{Unit.uncompleted_units_of_partially_completed_orders.count}", admin_units_path(order: 'id_desc', page: '1', scope: 'uncompleted_units_of_partially_completed_orders') }
          end
          tr do
            td { 'Orders Ready for Delivery' }
            td { link_to "#{Order.ready_for_delivery.count}", admin_orders_path(order: 'id_desc', page: '1', scope: 'ready_for_delivery') }
          end
          tr do
            td { 'Burn Orders to DVD' }
            td { link_to "#{Order.ready_for_delivery.has_dvd_delivery.count}", admin_orders_path(order: 'id_desc', page: '1', scope: 'ready_for_delivery', q: { dvd_delivery_location_id_is_null: 'true' }) }
          end
        end
      end
    end
    ...
    div class: 'three-column' do
      panel 'Outstanding Orders', priority: 5, toggle: 'show' do
        table do
          tr do
            td { 'Orders Due Today' }
            td { link_to "#{Order.due_today.count}", admin_orders_path(order: 'date_due_desc', page: '1', scope: 'due_today') }
          end
          tr do
            td { 'Orders Due In A Week' }
            td { link_to "#{Order.due_in_a_week.count}", admin_orders_path(order: 'date_due_desc', page: '1', scope: 'due_in_a_week') }
          end
          tr do
            td { 'Orders Overdue (< 1 yr)' }
            td { link_to "#{Order.overdue.count}", admin_orders_path(order: 'date_due_desc', page: '1', scope: 'overdue') }
          end
          tr do
            td { 'Invoices Overdue (30 days or more)' }
            td { link_to "#{Invoice.past_due.count}", admin_invoices_path(order: 'date_invoice_desc', page: '1', scope: 'past_due') }
          end
          tr do
            td { 'Invoices Overdue (2nd notice)' }
            td { link_to "#{Invoice.notified_past_due.count}", admin_invoices_path(order: 'date_invoice_desc', page: '1', scope: 'notified_past_due') }
          end
          tr do
            td { 'Customers with unpaid orders' }
            td { link_to "#{Customer.has_unpaid_invoices.count}", admin_customers_path(customer: 'id_desc', page: '1', scope: 'has_unpaid_invoices')  }
          end
        end
      end
    end
