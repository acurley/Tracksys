ActiveAdmin.register HeardAboutService do
  ...
  index do
    ...
    column :customers do |heard_about_service|
      link_to "#{heard_about_service.customers.size}", admin_customers_path(q: { heard_about_service_id_eq: heard_about_service.id })
    end
    column :orders do |heard_about_service|
      link_to "#{heard_about_service.orders.size}", admin_orders_path(q: { heard_about_service_id_eq: heard_about_service.id })
    end
    column :units do |heard_about_service|
      link_to "#{heard_about_service.units.size}", admin_units_path(q: { heard_about_service_id_eq: heard_about_service.id })
    end
    column :master_files do |heard_about_service|
      link_to "#{heard_about_service.master_files.size}", admin_master_files_path(q: { heard_about_service_id_eq: heard_about_service.id })
    end
