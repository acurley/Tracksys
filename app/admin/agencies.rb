ActiveAdmin.register Agency do
  ...
  index id: 'agencies' do
    ...
    column :customers do |agency|
      link_to "#{agency.customers.size}", admin_customers_path(q: { agencies_id_eq: agency.id })
    end
    column :requests do |agency|
      link_to "#{agency.requests.size}", admin_orders_path(q: { agency_id_eq: agency.id }, scope: 'awaiting_approval')
    end
    column :orders do |agency|
      link_to "#{agency.orders.size}", admin_orders_path(q: { agency_id_eq: agency.id }, scope: 'approved')
    end
    column :units do |agency|
      link_to "#{agency.units.size}", admin_units_path(q: { agency_id_eq: agency.id })
    end
    column :bibls do |agency|
      link_to "#{agency.bibls.size}", admin_bibls_path(q: { agencies_id_eq: agency.id })
    end
    column :master_files do |agency|
      link_to "#{agency.master_files.size}", admin_master_files_path(q: { agency_id_eq: agency.id })
    end
    column :descendants do |agency|
      # agency.descendants.map(&:names_depth_cache).map {|cache| cache.gsub("#{agency.name}/", '')}.sort || "N/A"
      agency.children.sort_by(&:name).each do|child|
        div { link_to "#{child.name}", admin_agency_path(child) }
      end
    end
    column 'Parents' do |agency|
      agency.ancestors.each do|ancestor|
        div { link_to "#{ancestor.name}", admin_agency_path(ancestor) }
      end
    end
    ...
    panel 'Detailed Information' do
      attributes_table_for agency do
        ...
        row :parent do |agency|
          agency.ancestors.each do|ancestor|
            div { link_to "#{ancestor.name}", admin_agency_path(ancestor) }
          end unless agency.ancestors.empty?
        end
        row :children do |agency|
          agency.children.sort_by(&:name).each do|child|
            div { link_to "#{child.name}", admin_agency_path(child) }
          end unless agency.children.empty?
        end
