ActiveAdmin.register HeardAboutResource do
  ...
  index do
    ...
    column :customers do |heard_about_resource|
      link_to "#{heard_about_resource.customers.size}", admin_customers_path(q: { heard_about_resources_id_eq: heard_about_resource.id })
    end
    column :orders do |heard_about_resource|
      link_to "#{heard_about_resource.orders.size}", admin_orders_path(q: { heard_about_resources_id_eq: heard_about_resource.id })
    end
    column :units do |heard_about_resource|
      link_to "#{heard_about_resource.units.size}", admin_units_path(q: { heard_about_resource_id_eq: heard_about_resource.id })
    end
    column :master_files do |heard_about_resource|
      link_to "#{heard_about_resource.master_files.size}", admin_master_files_path(q: { heard_about_resource_id_eq: heard_about_resource.id })
    end
