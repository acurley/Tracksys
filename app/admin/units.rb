ActiveAdmin.register Unit do
  ...
  batch_action :print_routing_slips do |_selection|
  end
    ...
    div class: 'columns-none' do
      if !unit.master_files.empty?
        panel 'Master Files', toggle: 'show' do
          table_for unit.master_files do |_mf|
  ...
  sidebar 'Delivery Workflow', only: [:show] do
    if File.exist?(File.join(IN_PROCESS_DIR, '%09d' % unit.id))
      ...
      if !unit.date_patron_deliverables_ready and not unit.intended_use == 'Digital Collection Buidling'
      ...
      if unit.date_archived && unit.date_patron_deliverables_ready and not unit.intended_use == 'Digital Collection Building'
        div { 'This unit has been archived and patron deliverables are generated.  There are no more finalization steps available.' }
      end
    else
      div { 'Files for this unit do not reside in the finalization directory. No work can be done on them.' }
    end
    ...
    if unit.date_archived
      ...
      div { 'This unit cannot be downloaded because it is not archived.' }
    end
  end
  ...
  sidebar 'Digital Library Workflow', only: [:show] do
    ...
    if unit.in_dl?
      if  unit.master_files.last.is_a?(MasterFile) && !unit.master_files.last.exists_in_repo?
  ...
  controller do
    ...
    caches_action :show, unless: proc { |c| File.exist?(File.join(IN_PROCESS_DIR, '%09d' % c.params[:id])) }
