ActiveAdmin.register Customer, namespace: :patron do
  ...
  index as: :table do
    ...
    column ('Bibliographic Records') do |customer|
      link_to customer.bibls.size.to_s, patron_bibls_path(q: { customers_id_eq: customer.id }) # Bibl requires 'customers_id' since there are potentially many customers for each bibl
    end
    column :master_files do |customer|
      link_to customer.master_files_count.to_s, patron_master_files_path(q: { customer_id_eq: customer.id })
    end
