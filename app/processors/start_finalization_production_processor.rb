class StartFinalizationProductionProcessor < ApplicationProcessor
  # Written by: Andrew Curley (aec6v@virginia.edu) and Greg Murray (gpm2a@virginia.edu)
  # Written: January - March 2010
  require 'fileutils'

  subscribes_to :start_finalization_production, :ack => 'client', 'activemq.prefetchSize' => 1
  ...
  def on_message(message)
    ...
    if !File.exist?(FINALIZATION_DROPOFF_DIR_PRODUCTION)
      on_failure "#{FINALIZATION_DROPOFF_DIR_PRODUCTION} directory does not exist.  Check mounts to NetApps."
    else
      contents = Dir.entries(FINALIZATION_DROPOFF_DIR_PRODUCTION).delete_if { |x| x == '.' || x == '..' || x == '.AppleDouble' }
