class UpdateUnitStatus < ApplicationProcessor
  ...
  def on_message(message)
    ...
    fail "Parameter 'unit_status is required" if hash[:unit_status].blank?
    fail "Parameter 'unit_status' is not a valid value" if Unit::UNIT_STATUSES.include?(hash[:unit_status])
