class SendUnitToArchiveProcessor < ApplicationProcessor
  ...
  def on_message(message)
    ...
    fail "Parameter 'internal_dir' is required" if hash[:internal_dir].blank?
    fail "Parameter 'source_dir' is required" if hash[:source_dir].blank?
    ...
    if @source_dir == "#{IN_PROCESS_DIR}"
      fail "Parameter 'internal_dir' must equal 'yes' if 'source_dir' == #{@source_dir}" unless @internal_dir == 'yes'
    ...
    # Second, messages coming from the start_manual processor
      ...
      if @internal_dir == 'yes'
        fail "Parameter 'unit_id' is required" if hash[:unit_id].blank?
        fail "Parameter 'unit_dir' is required" if hash[:unit_dir].blank?
      ...
      when File.file?(f)
        ...
        if /^\./ =~ basename
          ...
          File.open(f, 'r') do |file|
            source_md5.update(file.read(16_384)) until file.eof
          end

          # Get copy MD5
          copy_md5 = Digest::MD5.new
          File.open(File.join(ARCHIVE_WRITE_DIR, parent, basename), 'r') do |file|
            copy_md5.update(file.read(16_384)) until file.eof
          end
    ...
    if @errors.eql?(0)
      ...
      if unit && unit.archive
        archival_location = unit.archive.name if unit.archive.respond_to?(:name)
      end
      ...
      if @source_dir != "#{IN_PROCESS_DIR}" && @internal_dir == 'yes'
