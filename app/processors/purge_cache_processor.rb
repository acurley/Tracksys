class PurgeCacheProcessor < ApplicationProcessor
  ...
  def on_message(message)
    ...
    fail "Parameter 'subject_class' is required" if hash[:subject_class].blank?
    fail "Parameter 'subject_id' is required" if hash[:subject_id].blank?
    fail "Parameter 'associated_class' is required" if hash[:associated_class].blank?
  ...
  def purge_cache(subject, associated_class)
    ...
    when @relationship == :has_many || @relationship == :has_and_belongs_to_many
      ...
      @subject.send(@associated_class.classify.underscore.pluralize).each do|associated_object|
    ...
    when @relationship == :belongs_to || @relationship == :has_one
      ...
      expire_cache(associated_object) if associated_object
    else
