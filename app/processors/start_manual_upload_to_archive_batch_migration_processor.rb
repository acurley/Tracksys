...
class StartManualUploadToArchiveBatchMigrationProcessor < ApplicationProcessor
  ...
  def on_message(message)
    ...
    if !File.exist?(File.join(MANUAL_UPLOAD_TO_ARCHIVE_DIR_BATCH_MIGRATION, day))
      ...
      contents = Dir.entries(@original_source_dir).delete_if { |x| x == '.' || x == '..' }
          ...
          if !File.directory?(complete_path)
            ...
            if (Dir.entries(complete_path) == ['.', '..'])
              ...
              if regex_unit.match(content).nil?
                ...
                message = ActiveSupport::JSON.encode(unit_dir: content, source_dir: MANUAL_ARCHIVE_IN_PROCESS_DIR_BATCH_MIGRATION, internal_dir: @internal_dir)
                ...
                message = ActiveSupport::JSON.encode(unit_id: @unit_id, unit_dir: content, source_dir: MANUAL_ARCHIVE_IN_PROCESS_DIR_BATCH_MIGRATION, internal_dir: @internal_dir)
                publish :send_unit_to_archive, message
              end
            end
          end
