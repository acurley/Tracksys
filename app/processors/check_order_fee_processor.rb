class CheckOrderFeeProcessor < ApplicationProcessor
  ...
  def on_message(message)
    ...
    if @working_order.fee_estimated && !@working_order.fee_actual
      on_error "Error with order fee: Order #{@order_id} has an estimated fee but no actual fee."
    elsif @working_order.fee_actual && !@working_order.fee_estimated
    ...
    elsif @working_order.fee_estimated && @working_order.fee_actual
      if @working_order.fee_estimated.to_i.eql?(0) && !@working_order.fee_actual.to_i.eql?(0)
      ...
      elsif @working_order.fee_estimated.to_i.eql?(0) && @working_order.fee_actual.to_i.eql?(0)
