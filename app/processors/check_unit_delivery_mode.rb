class CheckUnitDeliveryModeProcessor < ApplicationProcessor
  ...
  def on_message(message)
    ...
    if @working_unit.include_in_dl && @working_unit.availability_policy_id? && @working_unit.intended_use.description == 'Digital Collection Building'
      @mode = 'dl'
    ...
    if not @working_unit.intended_use.description == 'Digital Collection Building' and !@working_unit.include_in_dl
      @mode = 'patron'
    ...
    if @working_unit.include_in_dl && @working_unit.availability_policy_id? and not @working_unit.intended_use.description == 'Digital Collection Building'
      @mode = 'both'
      on_success = "Unit #{@unit_id} requires the creation of patron deliverables."
      message = ActiveSupport::JSON.encode(unit_id: @unit_id, mode: @mode, source_dir: @source_dir)
      publish :copy_unit_for_deliverable_generation, message
    ...
    if !@working_unit.intended_use_deliverable_resolution && !@working_unit.intended_use_deliverable_format && !@working_unit.include_in_dl
