class QaFilesystemAndIviewXmlProcessor < ApplicationProcessor
  ...
  def check_xml_files
    ...
    if @xml_files.empty?
      ...
      (@error_messages << error_list).flatten! if error_list != []
      ...
      root.xpath('MediaItemList/MediaItem').each do|mediaitem|
        ...
        unless filename_regexp.match(filename)
        ...
        if colorprofile != 'Adobe RGB (1998)' && colorprofile != 'cruse-lr-picto' && colorprofile != 'Dot Gain 20%'
      ...
      if @working_unit.bibl.is_manuscript?
        ...
        if has_SetList == true
          ...
          if set_count == 1 && set_name && set_name.text == '@KeywordsSet'  # this strange value (some kind of placeholder?) occurs regularly in XML files created by iView; ignore it
  ...
  def handle_errors
    ...
    if @error_messages.empty?
      ...
      @error_messages.each do|message|
        logger.debug "QaFilesystemAndIviewXmlProcessor handle_errors >#{message.class}< >#{message}< "
        on_failure message
        if message == @error_messages.last
          on_error "Unit #{@unit_id} has failed the Filesystem and Iview XML QA Processor #{message}"
        end
      end
    end
  end
