module ExportIviewXML
  ...
  def create_iview_xml(arg = nil)
    ...
    else fail ArgumentError "#{__method__} expects a Component or Component.id as first argument!"
    end
    @guide = nil
    # build guide from top-level component
    if @component.respond_to?(:ancestor_ids)
      then @guide = @component.root
    else fail ArgumentError, "Component #{@component.id} must respond to :ancestor_ids"
    end
    fail ArgumentError, '' unless @guide.is_a?(Component)
  ...
  def add_component_data(component, xmlfragment)
    ...
    fail ArgumentError 'arg2 should be a <Set> element' unless xmlfragment.name == 'Set'
