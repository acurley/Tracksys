...
module ImportIviewXml
  ...
  def self.import_iview_xml(file, unit_id)
    ...
    if error_list != [] && !error_list.nil?
      fail ImportError, "qa_iview_xml found errors: #{error_list}"
    end
    ...
    root.xpath('MediaItemList').each do |list|
      list.xpath('MediaItem').each do |_item|
    ...
    doc.xpath('//processing-instruction()').each do |pi|
      if pi.name == 'iview' || pi.name == 'expression'
        format_software = pi.name
        matches = pi.text.match(/exportversion=["']([^"']*)["']/)
        format_version = matches[1] if matches
      end
    end
    ...
    begin
        ...
        root.xpath('MediaItemList').each do |list|
          list.xpath('MediaItem').each do |item|
            ...
            if MasterFile.find(:first, conditions: ['unit_id = :unit_id AND filename = :filename', { unit_id: unit_id, filename: master_file.filename }])
              fail ImportError, "Import failed for Unit #{unit_id}, because a Master File with filename '#{master_file.filename}' already exists for this Unit"
            end
            # if MasterFile object fails validity, raise error with custom error message
            unless master_file.valid?
              fail ImportError, "<MediaItem> with <UniqueID> \"#{iview_id}\" and <Filename> \"#{master_file.filename}\": #{master_file.errors.full_messages}"
            end
            ...
            unless image_tech_meta.valid?
              fail ImportError, "<MediaItem> with <UniqueID> \"#{iview_id}\": #{image_tech_meta.errors.full_messages}"
            end
      ...
      Rails.logger.info "[#{Time.now}] #{@current_caller}: MasterFile batch transaction completed (#{@pid_count} items)."
        ...
        if retry_attempts > 0
          ...
          raise report # will send failure message to bus, storing report in db
        end
        retry_attempts -= 1
    end
    ...
    unit.save
    Hash[master_file_count: @master_file_count]
  end
  ...
  def self.new_image_tech_meta(item, master_file_id)
    ...
    if element && element['unit'] && element['unit'].match(/^pixels$/i)
    ...
    capture_date.gsub(/(\d\d\d\d):(\d\d):(\d\d)/, '\1-\2-\3') if capture_date
    ...
    if element && element['unit'] && element['unit'].match(/^bits$/i)
  ...
  def self.qa_iview_xml(xml, unit = nil)
    ...
    xml.remove_namespaces! if xml.namespaces
